...
// OpenAI configuration is server-side only. The deployment environment should
...
const DEFAULT_CONTENT = {
  ...
  nav: {home:'Home', services:'Services', projects:'Projects', about:'About', process:'Process', contact:'Contact', estimate:'Get Estimate Quotation', consultation:'Book a Consultation'},
...
function defaultData(){
  ...
  return {version:8, admin:{name:process.env.ADMIN_NAME || 'MSC Admin', loginId:process.env.ADMIN_LOGIN_ID || 'MSCADMIN', ...p}, content:DEFAULT_CONTENT, projects:SAMPLE_PROJECTS, laminates:[], socials:{instagramUrl:'',facebookUrl:'',youtubeUrl:'',pinterestUrl:'',whatsappUrl:'',iconStyle:'circle'}, siteSettings:{demoVideoUrl:'',loaderDuration:4500}, emailSettings:{host:'',port:465,secure:true,user:'',pass:'',to:'mscinterior1@gmail.com',from:''}, leads:[]};
}
function readData(){
...
const defaultServiceImages=['https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?auto=format&fit=crop&w=1000&q=85','https://images.unsplash.com/photo-1556912167-f556f1f39fdf?auto=format&fit=crop&w=1000&q=85','https://images.unsplash.com/photo-1558997519-83ea9252edf8?auto=format&fit=crop&w=1000&q=85','https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?auto=format&fit=crop&w=1000&q=85','https://images.unsplash.com/photo-1600607688969-a5bfcd646154?auto=format&fit=crop&w=1000&q=85','https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&w=1000&q=85']; (data.content.services.items||[]).forEach((item,i)=>{if(!item.image)item.image=defaultServiceImages[i%defaultServiceImages.length];});
...
data.socials={instagramUrl:'',facebookUrl:'',youtubeUrl:'',pinterestUrl:'',whatsappUrl:'',iconStyle:'circle',...(data.socials||{})};
data.siteSettings={demoVideoUrl:'',loaderDuration:4500,...(data.siteSettings||{})};
data.emailSettings={host:'',port:465,secure:true,user:'',pass:'',to:'mscinterior1@gmail.com',from:'',...(data.emailSettings||{})};
...
const server=http.createServer(async(req,res)=>{
  ...
  // Laminate CRUD
  if(req.method==='POST' && url.pathname==='/api/laminates'){if(!requireAuth(req,res)) return;try{const b=await readJson(req,50000);if(!b.name||!b.image)return send(res,400,{ok:false,error:'Laminate name and image are required.'});const item={id:crypto.randomUUID(),name:String(b.name).slice(0,120),image:String(b.image),material:String(b.material||'').slice(0,120),specification:String(b.specification||'').slice(0,300),thickness:String(b.thickness||'').slice(0,80),finish:String(b.finish||'').slice(0,120)};data.laminates.unshift(item);writeData(data);return send(res,200,{ok:true,laminate:item});}catch(e){return send(res,400,{ok:false,error:'Could not add laminate finish.'});}}
  if(/^\/api\/laminates\//.test(url.pathname) && req.method==='PUT'){if(!requireAuth(req,res)) return;try{const id=decodeURIComponent(url.pathname.split('/').pop());const idx=data.laminates.findIndex(p=>p.id===id);if(idx<0)return send(res,404,{ok:false,error:'Laminate not found.'});const b=await readJson(req,50000);data.laminates[idx]={...data.laminates[idx],name:String(b.name||data.laminates[idx].name).slice(0,120),image:String(b.image||data.laminates[idx].image),material:String(b.material ?? (data.laminates[idx].material || '')).slice(0,120),specification:String(b.specification ?? (data.laminates[idx].specification || '')).slice(0,300),thickness:String(b.thickness ?? (data.laminates[idx].thickness || '')).slice(0,80),finish:String(b.finish ?? (data.laminates[idx].finish || '')).slice(0,120),id};writeData(data);return send(res,200,{ok:true,laminate:data.laminates[idx]});}catch(e){return send(res,400,{ok:false,error:'Could not update laminate.'});}}
